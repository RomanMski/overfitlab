...
\textbf{Costs are charged crudely and only on request.} The sweep accepts a rate in basis points and charges it against turnover, which requires the strategy to report its positions and is off by default. A flat rate is the crudest possible model. Real cost rises with size and worsens in the volatile periods a strategy is most likely to be trading, so a result that survives a flat charge has not been shown to survive execution. The two selection statistics take whatever return series they are given and will report on gross figures as readily as net ones.
