...
Two published corrections address this directly. The deflated Sharpe ratio \citep{bailey2014deflated} computes the Sharpe ratio the best of $N$ trials is expected to reach under a null of no skill, then asks whether the observed maximum clears it. The probability of backtest overfitting \citep{bailey2016pbo} takes a different route: it splits the history into blocks, forms every balanced in-sample and out-of-sample partition, selects the best configuration in-sample, and records how often that selection lands below the median of its peers out-of-sample. Related work on data snooping \citep{white2000reality} and on the multiple-testing burden in published factors \citep{harvey2015backtesting} makes the same point from other directions.
...
Let $r_{1:T}$ be the observed return series and let $A$ be a strategy mapping a return series to the returns it would have earned on it. Let $B_\ell$ denote a resampling scheme with expected block length $\ell$, so that runs of about $\ell$ consecutive observations are preserved and only their arrangement changes. We use the stationary bootstrap of \citet{politis1994stationary}, whose geometric block lengths leave the resampled series stationary, unlike the fixed-length moving block scheme of \citet{kunsch1989jackknife}.

At $\ell = 1$ the scheme reduces to independent resampling. Every autocorrelation, every volatility cluster and every trend is gone, while the marginal distribution is preserved exactly: the mean, the variance, the skewness and every individual extreme observation are those of the original series. This is worth stating plainly because it is easy to describe the shuffled series as noise, and it is not. It is the same returns in a different order. At large $\ell$ most local structure survives as well.
...
Buy and hold earns 0.36 on the observed path and between 0.33 and 0.39 at every block length, giving $D = -0.06$. This is not a failure of the method. It is the correct answer: buy and hold earns from drift, drift is a property of the marginal distribution, and resampling preserves the marginal distribution exactly. A flat curve is informative. It says the result does not depend on when anything happened, which for a strategy that claims to time the market would be damning.
