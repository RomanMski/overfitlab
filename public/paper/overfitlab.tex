...
At $\ell = 1$ the scheme reduces to independent resampling. Every autocorrelation, every volatility cluster and every trend is gone, while the marginal distribution is preserved exactly, so the mean, the variance, the skewness and every individual extreme observation are those of the original series. This is worth stating plainly because it is easy to describe the shuffled series as noise, and it is not. It is the same returns in a different order. At large $\ell$ most local structure survives as well.
